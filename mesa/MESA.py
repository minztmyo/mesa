...
class MESA:
    ...
    def __init__(self, manifestFilename, outputPrefix, args):
        ...
        print("Writing exclusion counts...")
        self.writeExclusions()
        print("\tDone", timer.check())
    ...
    def writeExclusions(self):
        """ """
        tab = '\t'
        with open(f"{self.outputPrefix}_exclusionCounts.tsv", "w") as exclusionTsv:
            exclusionTsv.write(f"cluster\t{tab.join([s.name for s in self.manifest])}\n")
            for i, junction in enumerate(sorted(self.clusters)):
                exclusions = np.zeros(len(self.manifest))
                for excluded in self.clusters[junction]:
                    exclusions += self.counts[self.junctionIndex[excluded], :]
                exclusionTsv.write(f"{self.junctionString(junction)}\t{tab.join([f'{x:.0f}' for x in exclusions])}\n")
...
def add_parser(parser):
    ...
    parser.add_argument("--minEntropy", type=float, default=1,
                        help="Shannon's diversity index associated with a junction, minimum required for inclusion [Default 1]")
